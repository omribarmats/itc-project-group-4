	...
	static async Search(req, res) {
		...
		if (isQuestion.includes('false')) {
			...
			return res.status(200).json({
				success: true,
				destinations: 'Please enter a question',
			});
		}
		...
		if (isDestination.includes('false')) {
			...
			return res.status(200).json({
				success: true,
				destinations:
					'Hey there! Please enter a question about a particular destination. For instance: "Could you suggest a great spot with a view to see Lisbon, Portugal?"',
			});
		}
		...
		if (isDestination.includes('false')) {
			...
			return res.status(200).json({
				success: true,
				destinations: 'Please enter ask for recommendations',
			});
		}
