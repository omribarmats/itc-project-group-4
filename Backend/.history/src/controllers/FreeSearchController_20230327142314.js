...
module.exports = class FreeSearchController {
	static async Search(req, res) {
		...
		prompt = `Give top two travel recommendations based on the following text "${query}" in JSON, for example: 
...
`;
		...
		try {
			...
			const parseJason = JSON.parse(giveRecommendation);

			return res.status(200).json({
				success: true,
				destinations: parseJason,
			});
		} catch (error) {
