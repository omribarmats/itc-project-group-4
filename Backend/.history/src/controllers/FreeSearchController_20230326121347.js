...
module.exports = class FreeSearchController {
	static async Search(req, res) {
		...
		if (isQuestion.includes('false')) {
			console.log('is a question?', isQuestion);
			return res.status(200).json({
				success: true,
				data: 'Please enter a question',
			});
		}
		prompt = `Is the following text a destination on earth? "${query}" Return "true/"false"}`;
		const isDestination = (
			await Chat(prompt, 0.1, 10, 0.1, 0.3, 0.3)
		).toLowerCase();

		if (isDestination.includes('false')) {
			console.log('is not a destination?', isDestination);
			return res.status(200).json({
				success: true,
		...
		prompt = `Is the following text is asking about for travel recommendations "${query}" Return "true/"false"}`;
