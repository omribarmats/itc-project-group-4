		...
		try {
			...
			if (isQuestion.includes('false'))
				return res.status(200).json({
					success: true,
					data: 'Please enter a question about a travel destination',
				});
			...
			isDestination = await Chat(prompt, 0.1, 10, 0.1, 0.3, 0.3).toLowerCase();
			if (isDestination.includes('false')) {
				console.log('Is not a destination', isDestination);
			} else {
				console.log('Is a destination', isDestination);
			}
